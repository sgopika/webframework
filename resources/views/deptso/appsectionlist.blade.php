    ...
    <div class="responsive">
          <table class="table table-stripped table-sm table-hover box-shadow--6dp" id="resposivetable">
            ...
            <tbody class="eng_xxxs">
              ...
              <button type="button" class="revert btn btn-sm bg-darkBrown fg-lightGray eng_xxxxs" name="revert" id="{{ $res->id }}" data-toggle="tooltip" data-placement="top" title="revert"><i class="fas fa-ban"></i>&nbsp;revert</button>
...
$(document).on('click', '.verify', function(){
  var id = $(this).attr('id');
      ...
      var action_url = '';
      if(usertype==14){
         action_url = "/depthead/appsectionview/"+id;

      } else if(usertype==16){
         action_url = "/deptso/appsectionview/"+id;
      } 
      $.ajax({
         url: action_url,
         dataType:"json",
...
$(document).on('click', '.revert', function(){
  var id = $(this).attr('id');
      ...
      var action_url = '';
      if(usertype==14){
         action_url = "/depthead/appsectionrevert/"+id;

      } else if(usertype==16){
         action_url = "/deptso/appsectionrevert/"+id;
      } 
      $.ajax({
         url: action_url,
         dataType:"json",
...
$('#ajaxmodalform').on('submit', function(event){ 
    ...
    var action_url = '';
    if(usertype==14){
       action_url = "{{ route('depthead.appsectionlistupdate') }}";
    } else if(usertype==16){
       action_url = "{{ route('deptso.appsectionlistupdate') }}";
    } 
